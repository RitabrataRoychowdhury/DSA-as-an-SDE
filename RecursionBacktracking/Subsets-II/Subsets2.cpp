...
public:
    vector<vector<int>> subsetsWithDup(vector<int> &nums)
    {
        ...
        vector<int> temp;
        binaryRecurse(nums, res, temp, included, 0);
        return res;
    }
    void binaryRecurse(vector<int> &nums, vector<vector<int>> &res, vector<int> &temp, vector<bool> &included, int index)
    {
        if (nums.size() == index)
        {
            res.push_back(temp);
        ...
        binaryRecurse(nums, res, temp, included, index + 1);
